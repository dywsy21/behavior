...
def _save_resumable_training_checkpoint(
    ...
    *,
    ...
    epoch: int,
    batch_idx: int,
    action_batch_idx: int,
    model,
    optimizer,
    scheduler,
    ema_model=None,
):
    """Save all state required for cfg.resume_ckpt."""
...
def _training_is_complete(step: int, max_steps: int) -> bool:
    """Return whether this run must make zero further optimizer updates."""
...
def finetune(cfg: DictConfig):
    ...
    # A task may explicitly disable a complete model branch (for example the
    # continuous FM action expert in a pure-AR recipe). Freeze before DDP is
    # constructed: non-trainable parameters are then absent from its reducer and
    # from the optimizer param groups. The full name/count audit is persisted so
    # a run is reproducible and a typo cannot silently leave a branch trainable.
    _freeze_audit = None
    _freeze_prefixes = list(cfg.model.get("freeze_parameter_prefixes", []))
    if _freeze_prefixes:
        _freeze_audit = freeze_parameters_by_prefixes(model, _freeze_prefixes)
        _freeze_audit["ddp_exclusion"] = (
            "frozen before DDP construction (requires_grad=False)"
        )
        if accelerator.is_main_process:
            _audit_path = output_dir / "parameter_audit.json"
            _audit_path.write_text(json.dumps(_freeze_audit, indent=2) + "\n")
            logger.info(
                "[Parameter Freeze Audit] prefixes=%s; frozen=%d tensors / %d numel; "
                "trainable=%d tensors / %d numel; audit=%s",
                _freeze_audit["requested_prefixes"],
                _freeze_audit["frozen_parameter_tensors"],
                _freeze_audit["frozen_parameter_numel"],
                _freeze_audit["trainable_parameter_tensors_after_freeze"],
                _freeze_audit["trainable_parameter_numel_after_freeze"],
                _audit_path,
            )
    ...
    if stats_path is not None:
        ...
        if accelerator.is_main_process:
            # An unchanged precomputed source is read-only. Only persist a
            # union back to it when new embodiment statistics were computed.
            if any_missing_globally:
    ...
    recovery_cfg = cfg.get("memlite_recovery", {})
    if recovery_cfg.get("enabled", False):
        from g05.data.memlite_recovery_dataset import RecoveryAugmentedDataset
        train_dataset = RecoveryAugmentedDataset(
            train_dataset, recovery_cfg["manifest"], train_processor, split="train")
        # Preserve the original 5% validation measure for comparable periodic
        # loss. Recovery heldout examples are evaluated separately by source
        # instance; never silently mix their oversampling ratio into old eval.
        logger.info("Real recovery augmentation: %s examples, identity=%s",
                    len(train_dataset.entries), train_dataset.recovery_identity)
    ...
    memlite_sampling_cfg = cfg.data.get("memlite_branch_sampling", {})
    if isinstance(memlite_sampling_cfg, DictConfig):
        memlite_sampling_cfg = OmegaConf.to_container(memlite_sampling_cfg, resolve=True)
    memlite_sampling_cfg = dict(memlite_sampling_cfg or {})
    use_memlite_branch_sampler = bool(memlite_sampling_cfg.get("enabled", False))
    if use_memlite_branch_sampler:
        if overfit_batch is not None:
            raise ValueError("MEM-Lite branch-balanced sampling is incompatible with overfit_batch")
        train_sampler = build_memlite_train_sampler(
            train_dataset,
            sampling_config=memlite_sampling_cfg,
            batch_size=cfg.model.batch_size,
            num_replicas=sampler_num_replicas,
            rank=sampler_rank,
            seed=cfg.seed,
            shuffle=overfit_shuffle,
            high_fraction=float(memlite_sampling_cfg.get("high_fraction", 0.125)),
            drop_last=bool(memlite_sampling_cfg.get("drop_last", False)),
        )
        logger.info(
            "MEM-Lite branch-balanced sampler: high=%s/%s (%.1f%%), low=%s/%s",
            train_sampler.high_per_batch,
            cfg.model.batch_size,
            100.0 * train_sampler.high_per_batch / cfg.model.batch_size,
            train_sampler.low_per_batch,
            cfg.model.batch_size,
        )
        if hasattr(train_sampler, 'quotas'):
            logger.info("MEM-Lite motion/recovery sampler: tasks=%s, low quotas=%s, one recovery HL every %s HL draws; index=%s",
                        train_sampler.task_ids, train_sampler.quotas, train_sampler.high_recovery_period,
                        memlite_sampling_cfg['sampling_index_path'])
        elif hasattr(train_sampler, 'critical_per_batch'):
            logger.info(
                "MEM-Lite task-equal/event sampler: tasks=%s, critical=%s, other_low=%s; index=%s",
                train_sampler.task_ids, train_sampler.critical_per_batch,
                train_sampler.other_per_batch, memlite_sampling_cfg['sampling_index_path'],
            )
    else:
    ...
    if use_memlite_branch_sampler:
        eval_high_fraction = memlite_sampling_cfg.get(
            "eval_high_fraction", memlite_sampling_cfg.get("high_fraction", 0.125)
        )
        if eval_high_fraction is None:
            eval_high_fraction = memlite_sampling_cfg.get("high_fraction", 0.125)
        eval_sampler = ResumableDistributedBranchBalancedBatchSampler(
            eval_dataset,
            ...
            seed=cfg.seed,
            shuffle=True,
            high_fraction=float(eval_high_fraction),
            drop_last=False,
            low_only=memlite_sampling_cfg.get("strategy") == "fm_motion",
        )
    else:
        eval_sampler = DistributedSampler(
            ...
            seed=cfg.seed,
            shuffle=True,
        )  # PeriodicEvaluator advances sampler epoch when its iterator wraps.
    if overfit_batch is None and accelerator.is_main_process:
        logger.info(
            "Periodic eval uses a deterministic shuffled %s "
            "(seed=%s, initial_epoch=0); each iterator wrap advances the epoch "
            "to avoid task-prefix evaluation bias.",
            "MEM-Lite branch-balanced sampler" if use_memlite_branch_sampler else "DistributedSampler",
            cfg.seed,
        )
    ...
    if use_memlite_branch_sampler:
        # Mixed MEM-Lite teacher forcing is deliberately fail-fast in the
        # policy, so periodic eval must use the same per-batch branch balance.
    ...
    # Validate before any optimizer update. The helper returns None at step 0
    # after validating the interval, which is exactly what is wanted here.
    checkpointing_steps = cfg.get("checkpointing_steps", None)
    checkpoint_save_reason(
        step=0,
        max_steps=max_steps,
    ...
    if _freeze_audit is not None:
        _frozen_names = set(_freeze_audit["frozen_parameter_names"])
        _frozen_ids = {
            id(parameter)
            for name, parameter in _optim_model.named_parameters()
        ...
        _unexpected_optimizer_ids = _frozen_ids & _optimizer_ids
        if _unexpected_optimizer_ids:
            raise RuntimeError(
                "Frozen pure-AR parameters leaked into the optimizer: "
                f"{len(_unexpected_optimizer_ids)} tensors"
            )
        _freeze_audit["optimizer_exclusion_verified"] = True
        _freeze_audit["optimizer_parameter_tensors"] = len(_optimizer_ids)
        if accelerator.is_main_process:
            _audit_path.write_text(json.dumps(_freeze_audit, indent=2) + "\n")
            logger.info(
                "[Parameter Freeze Audit] optimizer exclusion verified: %d frozen tensors "
                "are absent from %d optimizer tensors",
                len(_frozen_ids),
                len(_optimizer_ids),
            )
    ...
    training_done = _training_is_complete(step, max_steps)
    if training_done and accelerator.is_main_process:
        logger.info(
            f"Training is already complete at step {step}/{max_steps}; skipping optimizer updates."
        )
        ...
        while not training_done:
            ...
            while batch_idx < len(train_dataloader):
                ...
                if recovery_cfg.get("enabled", False):
                    ...
                    record_batch_receipt(batch, output_dir, rank=accelerator.process_index,
                                         step=step+1, batch_index=action_batch_idx)
                ...
                if EVAL_TOKENIZER_FIRST_BATCH and cfg.model.get("eval_tokenizer_first_batch", True) and batch_idx == 0 and step == 0:
                    _inner_model = unwrap_model(model)
                ...
                # Save one full, resumable checkpoint after an optimizer step.  A
                # periodic final step is intentionally saved once, while a non-periodic
                # final step is saved here before epoch/action_batch_idx are advanced.
                checkpoint_reason = (
                    checkpoint_save_reason(
                        step=step,
                        max_steps=max_steps,
                ...
                if checkpoint_reason is not None and not _dry_run:
                    if accelerator.is_main_process:
                        logger.info(
                            f"Saving {checkpoint_reason} resumable checkpoint for step {step} ..."
                        )
