"""Explicit-step trained checkpoint -> six-actual-frame native FM for B subgoals.

No oracle windows enter this service. The controller owns subgoal origin, and
only the evaluator can report task success. A2 neural math matches its frozen
training source; the previously validated target-free native input/output
adapter is added. No legacy one-frame or partial LoRA restore is accepted.
"""
...
def validate_checkpoint_path(run, checkpoint, step):
    if type(step) is not int or step < 1 or checkpoint.resolve() != (run / f"checkpoints/step_{step}.pt").resolve():
        raise ValueError("Checkpoint must match this run and explicit step")
...
def main():
    ...
    for name, expected in padding_patch['files'].items():
        if sha(args.runtime/name) != expected:
    ...
    receipt = dict(service_entry_sha256=sha(__file__), runtime_root=str(args.runtime), kind="formal_a2_native_skill_fm_history6_service", checkpoint=str(args.checkpoint),
        ...
        model_components=["low"], subgoal_origin="external_controller", not_success_rate=True,
        ...
        history_runtime_files={name:sha(args.runtime/name) for name in
    ...
    class FormalALowService(NativeLowOnlyService):
        ...
        def __init__(self):
            ...
            self.history_ingress=LowHistoryIngress()
