...
def prepare(folder,output,start,expected_manifest,counts_path,excluded):
    ...
    window.update(window_id=f"h09v-train-t{row['task']}-i{row['instance']}-paid{start}",max_steps=start+421,
    ...
    manifest={"schema":SCHEMA,"status":"PREPARED_NOT_COLLECTED_NOT_TRAINING_DATA","source":[row[k] for k in ("task","episode","instance")],
              ...
              "native_controls_max":420,"max_teacher_primitives":12,"new_controls":0,"new_resets":0,"training_eligible":False}
...
def verify_prepared(prepared,release):
    ...
    expected=dict(original);expected.update(window_id=f"h09v-train-t{row['task']}-i{row['instance']}-paid{len(prefix)}",
        max_steps=len(prefix)+421,prefix_actions_path=str(prepared/"prefix.npy"),prefix_actions_sha256=m["files_sha256"]["prefix.npy"])
...
if __name__=="__main__":
    ...
    a=p.parse_args();print(json.dumps(prepare(a.reference,a.output,a.start_control,a.reference_manifest_sha,a.counts,json.loads(a.held_out_groups))))
