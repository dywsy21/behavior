...
def validate_checkpoint(training,data_sha):
    ...
    if (result.get("status")!="COMPLETE" or result.get("optimizer_updates")!=120 or result.get("protocol")!=VERSION or
            ...
            identity.get("protocol")!=VERSION or identity.get("old_adapter_loaded") is not False or identity.get("base_model")!=BASE):
...
def main():
    ...
    if (auth.get("authorize_service") is not True or auth.get("code_commit")!=code or auth.get("dataset_sha256")!=data_sha or
            ...
            auth.get("protocol")!=VERSION or auth.get("physical_gpu")!=3 or auth.get("max_calls")!=48 or auth.get("port")!=8919):
    ...
    identity={"protocol":VERSION,"code_commit":code,"dataset_sha256":data_sha,"adapter_sha256":checkpoint["adapter_sha256"],
        **base_files,"physical_gpu":3,"port":8919,"training_result_sha256":sha(a.training/"result.json"),
        ...
        "max_calls":48,"tokens":list(TOKENS),"instructions":instructions,"variants":["base","finetuned"],"old_adapter_loaded":False}
