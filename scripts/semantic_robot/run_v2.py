...
def bounded_gate():
    return [HOLD, Action("right","up"), Action("right","down"),
...
def main():
    ...
    if not (0 <= args.prefix <= 448 and args.max_decisions in range(1,49) and 1 <= args.max_controls <= 1536 and 1 <= args.max_seconds <= 1200):
        raise ValueError("H-06 budget exceeded")
    ...
    if args.mode == "agent":
        ...
        if {g["task"] for g in gates} != {0,3} or not all(g["gate_ok"] and g["implementation_digest"] == digest for g in gates):
    ...
    policy = VLMPolicy(args.uri,args.expected_revision,max_calls=1+2*args.max_decisions) if args.mode=="agent" else None
    ...
    try:
        with OfficialEvaluatorSession(window,gpu=args.gpu) as environment:
            ...
            first = prepare_views(environment.observation()["images"],model,state.q)
            ...
            for decision in range(args.max_decisions):
                if controls>=args.max_controls or time.perf_counter()-started>=args.max_seconds or terminal or (manager and manager.stop_reason):
                    break
                if not policy and decision >= len(gate): break
                state = state_now()
                bundle = prepare_views(environment.observation()["images"],model,state.q,previous)
                ...
                if manager: manager.executed(action,feedback)
