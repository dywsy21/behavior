"""One-shot H44 stages, after independent review; never retry a physical run.
...
SOURCE = Path('/mnt/sdc1/robodojo/behavior_dev/git_worktrees/semantic_nearpose_197c2ba')
COMMIT = '197c2ba7bc941c9850e71217164f1fd9538b03cb'
DIGEST = 'e5ce7b5843c495a33fca56c247888f6733988b739397fc4f7931d5fd15596b30'
REVISION = '1d4bf0f2ff6012fd82039f2fa52739d0dd7c60c0'
ROOT = Path('/mnt/nvme_tmp/robodojo_agentic_20260921/h44_reachability')
...
OLD_RUN = Path('/mnt/nvme_tmp/robodojo_agentic_20260921/h38_appearance')
...
def gate_review(name):
    ...
    path = ROOT / f'gate_{name}_h44/result.json'
    result = read(path)
    ...
    for key in ('gripper_completion_v1', 'odometry_match_refinement', 'workspace_posture',
                ...
                'joint_boundary_start_v1', 'approach_translation_preview', 'near_pose_gap'):
...
def command_for(stage):
    if stage == 'model':
        ...
        policy = stage == 'policy'
        output = ROOT / ('radio_h44_fullstart' if policy else f'gate_{stage}_h44')
        ...
        if policy:
            ...
            for name in ('radio', 'plates'):
                command += ['--gate-result', str(ROOT / f'gate_{name}_h44/result.json')]
...
def main():
    ...
    for stage in ('policy', 'model'):
        require(not Path('/proc', str(read(OLD_RUN / f'launch_{stage}.json')['pid'])).exists(),
                'Old H38 process remains; preserve its evidence')
    ...
    if args.stage == 'model':
        ...
        require(proc.joinpath('cwd').resolve() == SOURCE and str(ROOT / 'server_h44').encode()
