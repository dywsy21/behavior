...
class ChunkedPolicyWrapper:
    ...
    async def get_action(self, raw_obs: dict) -> tuple[dict, str | None]:
        ...
        if self.need_obs:
            ...
            # Shared-inferencer diagnostics are metadata, never action arrays.
            # The MEM server records these in its trace; this generic server
            # must also exclude them from its per-step action cache.
            action.pop("_normalization_diagnostics", None)
            action.pop("_state_anchor_diagnostics", None)
