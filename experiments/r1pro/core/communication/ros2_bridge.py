...
class Ros2Bridge:
    def __init__(self, config, use_recv_time: bool = False, num_threads: int = None):
        ...
        self.subscribers = {}
        self.publishers = {}
        self.dof_of_arm = 7#cfg.data.shape_meta["state"][0]["raw_shape"] # left arm meta
        self.obs_buffer = {}
    ...
    def set_reset_pose(self, init_pose: dict):
        """Set the target pose for reset, loaded from initial_joints.json."""
        self._reset_pose = init_pose

    def reset(self, publish_count: int = 300, publish_freq: float = 100.0):
        """Reset R1_PRO to the initial pose defined in initial_joints.json.

        Publishes the target joint positions repeatedly at a fixed frequency so
        the robot's motion controller smoothly drives to the initial pose.

        Args:
            publish_count: Number of messages to publish (default 300).
            publish_freq:  Publishing frequency in Hz (default 100 Hz → 3 s total).
        """
        if self._reset_pose is None:
            raise RuntimeError(
                "reset() called but no reset pose is set. "
                "Ensure initial_joints_path is configured and the file exists."
            )
        logger.info("Resetting R1_PRO to initial pose...")
        interval = 1.0 / publish_freq
        for _ in range(publish_count):
            self.publishers[self.topics_config.action["left_arm"]].publish(
                array_to_joint_state(self._reset_pose["left_arm"]))
            self.publishers[self.topics_config.action["right_arm"]].publish(
                array_to_joint_state(self._reset_pose["right_arm"]))
            self.publishers[self.topics_config.action["left_gripper"]].publish(
                array_to_joint_state([self._reset_pose["left_gripper"]]))
            self.publishers[self.topics_config.action["right_gripper"]].publish(
                array_to_joint_state([self._reset_pose["right_gripper"]]))
            time.sleep(interval)
        logger.info("R1_PRO reset complete.")
    ...
    def state_callback(self, msg: JointState, _stack=None, state_name: str="None"):
        if state_name == "chassis":
            ...
            arm_dims = 7#self.shape_meta["state"][state_name]["raw_shape"]
            data_dict = self._create_data_dict(
                timestamp=header_stamp_to_timestamp(msg.header.stamp),
