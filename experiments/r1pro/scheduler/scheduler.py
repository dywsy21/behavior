...
class Scheduler:
    ...
    def __init__(self, config, recorder=None, binarize_gripper: bool = False, gripper_threshold: float = 0.5, visualize: bool = False):
        ...
        self._load_and_set_reset_pose(config)
        self.cnt = 0
    ...
    def _load_and_set_reset_pose(self, config: dict):
        path = config.get("robot", {}).get("initial_joints_path")
        if not path:
            raise RuntimeError(
                "Missing 'initial_joints_path' in [robot] section of config.toml. "
                "Please set it to the path of your initial_joints.json file."
            )
        if not os.path.isabs(path):
            # Resolve relative to the directory where run.py is located (experiments/r1pro/).
            base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            path = os.path.join(base, path)
        if not os.path.exists(path):
            raise RuntimeError(
                f"initial_joints.json not found at '{path}'. "
                "Please create the file with the safe initial joint positions for your robot."
            )
        with open(path) as f:
            init_pose = json.load(f)
        self.ros2_bridge.set_reset_pose(init_pose)
        logger.info("R1_PRO: reset pose loaded from %s", path)
