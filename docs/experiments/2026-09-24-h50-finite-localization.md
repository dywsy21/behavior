...
独审发现只检查spec hash会漏掉live FK数组原地修改，已修并加五类变异测试。15新接口测试＋5旧affordance通过，核心修后独审通过。探针8新测试＋旧共享44测试通过，独审待。机器人完整标定约45.2MiB，故仅SHA固定的标定文件允许64MiB，其余文件限32MiB；没有扩大GPU预算。
