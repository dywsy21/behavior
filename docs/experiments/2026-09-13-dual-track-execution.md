...
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 组误差复算完成，暂不选固定控制组权重。EMA1707751（d28581a/264 CPU）已等原CoT；独立尾端日程1820879（2300c50/299 CPU，恒定父末端LR）已等EMA，两者各原A4新Adam/5门＋独立500、不组合、0GPU/0各自更新；不重排。实际方法效果、LoRA容量及必要梯度依据仍未完成 |
...
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种AR500均完成但自由0/5完整；marker193 Adam/5门passed，正式最近302/500，100→200→300自由完整0/5→2/5→4/5，300 CE4.7525；task1/3同名窗口RMSE暂高于200点，不是内容收益/SR/部署，原500未完。schema十窗内容FM胜7/10；native唯一448＋256物理及29视图审查完成，未抓住/非持续绕圈，不重复。原CoT220792仍等marker（d114581/165 CPU，5＋500），无CoT正式结果，不重排；下一内容/闭环验收 |
