...
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 组误差/容量只读谱已完成（0bb9958d…/17 CPU）；新增36模块门尚未实现。19:18原EMA1707751/tail1820879因CoT被人为kill连锁退出，均0更新，不是方法训练失败；协调GPU使用前不重新排队，旧v1保留，尚无v2。实际容量、EMA/日程效果及必要任务梯度证据仍未完成 |
...
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种旧AR500及marker500/十窗均已验收，marker格式10/10但heldout误差较FM高26.98%，不升默认/扩该物理；旧schema/native物理审查不重做。CoT在19:18:02被账号ssy主动kill进程组，最后记录39/500、formal无checkpoint，不能无损续训/称方法失败；EMA/tail连锁退出。待协调GPU及明确恢复预算，不覆盖v1/擅自重跑。CoT内容/闭环与最终AR路线未完 |
