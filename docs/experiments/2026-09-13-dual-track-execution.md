...
| M-04 | 离散动作监督及梯度隔离是否有效 | 区分纯FM、CE+FM无隔离、CE+FM有隔离；同数据来源，先梯度/无泄漏和可学习性门 | 完整原参考门恢复通过，旧失败保留；FM v3正式400/500（12:41），joint4129515→KI4129539真实等待。四rank前188更新来源/时钟除路线标签外与旧control一致，不冒称增广像素一致。三臂各十状态的纯FM动作检查44255a4/161 CPU已备、0实际生成，须各500回读后运行；最终方法效果仍待验 |
