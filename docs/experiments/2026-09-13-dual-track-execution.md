...
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 组误差已完成，暂不选固定组权重；容量只读谱/实际覆盖完成（0bb9958d…/17 CPU），下一先验证新增18层QKV/out共36模块、原r8及旧192恢复/父函数保持/真梯度，未登记新500。EMA1707751（d28581a/264 CPU）等原CoT，tail1820879（2300c50/299 CPU）等EMA，各原5＋500/不组合/0更新，不重排。实际容量、EMA/日程效果及必要任务梯度证据仍未完成 |
...
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种旧AR500自由0/5完整；marker500/完整保存与十窗配对验收结束（3801388d…/831cbd40…），10/10自由完整但heldout前段误差比M04 FM高26.98%，不升默认/扩marker物理，不再重跑十窗。schema旧十窗/native448＋256及29视图已完成不重复。原CoT220792已正式2032144（d114581/5＋500，smoke完整门通过），EMA/tail原等待，不重排；CoT内容/闭环与最终AR路线验收未完 |
