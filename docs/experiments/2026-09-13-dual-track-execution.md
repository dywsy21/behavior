...
| M-02 | Beta分层/执行段加权各自效果 | 对同一对照一次只改一项，最多500更新/候选；固定train与原80eval，不改评估函数 | Beta真实两更新门通过；四卡入口因继承空CUDA失败，smoke/formal均0。v1两臂退出，7572ce2修复后准备显式v2恢复，原有限预算不变 |
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 待方法证据后选定具体配方 |
| M-04 | 离散动作监督及梯度隔离是否有效 | 区分纯FM、CE+FM无隔离、CE+FM有隔离；同数据来源，先梯度/无泄漏和可学习性门 | KI/joint真实梯度/两临时更新/suffix无泄漏通过；三臂v1因Beta前驱失败退出，0实际训练，待同一有限队列v2恢复 |
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | A4/native-task各500均完成，CE6.7695/6.7388，但自由均0/5完整。marker原切分v1因依赖失败退出/0训练，拟同预算v2。schema10/10是强制格式；同窗FM误差较低7/10，非SR。native十状态观察/原逆变换门通过，实际神经→wire/闭环、CoT正式训练未完成 |
