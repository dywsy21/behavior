...
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 组误差复算完成（64e9ff8/51 CPU、0新模型调用），少量窗口不足以选固定组权重。EMA接入前CPU小例子已证只恢复平均权重会重置时钟并覆盖历史，当前实验入口无EMA，后续需完整状态/显存门；这不是EMA训练效果。容量/EMA/日程仍待M-04等证据后选具体配方，非整体完成 |
...
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种AR500均完成但自由0/5完整；marker原193 Adam/5步保存门passed（d4840ca4…），正式1563012于17:20实际初始化/初评，原500尚未完成。schema十窗内容FM胜7/10；native观察/神经/wire/socket及唯一448＋256物理回合已完成，全动作/96锚点/18物理事件/29视图审查通过。短预算未抓住、手臂近保持、非持续绕圈；不是完整SR/等预算对照，不追加此回合。唯一native Subtask-CoT220792仍真实等待marker，d114581/165 CPU、原5＋500/原native-task500对照、不重做工程门；尚无正式CoT结果，勿重复提交 |
