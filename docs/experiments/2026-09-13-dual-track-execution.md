...
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种AR500均完成但自由0/5完整；marker v3 4129562已真实等待KI，原5＋500尚未训练。schema十窗内容FM胜7/10；native观察/神经/wire/socket及唯一448＋256物理回合已完成，全动作/96锚点/18物理事件/29视图审查通过。短预算未抓住、手臂近保持、非持续绕圈；不是完整SR/等预算对照，不追加此回合。CoT正式训练未完成 |
