...
| M-02 | Beta分层/执行段加权各自效果 | 对同一对照一次只改一项，最多500更新/候选；固定train与原80eval，不改评估函数 | 两v2均500/回读通过，固定80低1.1355%/0.2426%，全采样一致；30实际FM生成完成，五heldout前0:16 RMSE Beta较control差约2.27%、exec好约0.34%，混合信号，匹配闭环和组合适用性待验，不重训本两臂 |
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 待方法证据后选定具体配方 |
| M-04 | 离散动作监督及梯度隔离是否有效 | 区分纯FM、CE+FM无隔离、CE+FM有隔离；同数据来源，先梯度/无泄漏和可学习性门 | v2失败保留；单窗8次＋四卡80/160全部复现原值，阈值未改，原异常未复现/不宣称唯一根因。8fcf61f/111 CPU后FM v3已提交4092082，复用旧smoke验收、原A4新500；joint/KI随后按原预算恢复，真实阶段见plan |
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种AR500均完成但自由0/5完整；marker v2因前驱失败退出、0训练，修参考门后恢复原预算。schema十窗内容FM胜7/10；native观察/神经/wire/socket及唯一448＋256物理回合已完成，全动作/96锚点/18物理事件/29视图审查通过。短预算未抓住、手臂近保持、非持续绕圈；不是完整SR/等预算对照，不追加此回合。CoT正式训练未完成 |
