...
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 组误差复算完成，暂不选固定控制组权重。EMA完整状态/还原接入d28581a及264 CPU通过，唯一1707751已等待原CoT（spec d195fd58…、0GPU/0更新），原A4纯FM、新Adam、5门＋独立500，online/EMA配对；不要重新排队。真实EMA方法效果、容量/独立日程仍未完成 |
| M-04 | 离散动作监督及梯度隔离是否有效 | 区分纯FM、CE+FM无隔离、CE+FM有隔离；同数据来源，先梯度/无泄漏和可学习性门 | 本轮三臂500/全来源与30同状态动作首筛完成；joint/KI固定80比FM高0.96%/1.00%，heldout前段动作误差高2.59%/2.74%，均不升默认/不扩radio。全部输入指纹配对passed（KI8e5434b6…），不重复30生成。分模块裁剪44 CPU门完成但未接trainer/未证因果；M-04无新SR证据，后续具体方法/兼容仍须单项验收 |
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种AR500均完成但自由0/5完整；marker193 Adam/5步保存门passed（d4840ca4…），正式1563012最近125/500，100点CE5.5262/free0/5，body0已出现但有重复/缺组，原500尚未完成。schema十窗内容FM胜7/10；native唯一448＋256物理回合及29视图审查完成，未抓住、手臂近保持、非持续绕圈；不是完整SR/等预算对照，不追加此回合。唯一native Subtask-CoT220792仍等marker，d114581/165 CPU、原5＋500/原native-task500对照、不重做工程门；尚无正式CoT结果，勿重复提交 |
| BOTH-01 | 有效方法组合是否保留收益 | 已验证候选才组合；相同新起点/seed的局部与完整任务对照，逐项消融有害交互 | 等M-03/AR剩余证据；M-02没有局部成功收益，M-04双监督初筛无益，不直接叠加这些候选；尚未声称有效组合 |
