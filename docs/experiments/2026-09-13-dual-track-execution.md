...
| M-02 | Beta分层/执行段加权各自效果 | 对同一对照一次只改一项，最多500更新/候选；固定train与原80eval，不改评估函数 | v1失败保留；7572ce2/267 CPU后，Beta v2实际两更新/四卡通信/5更新完整保存门通过，formal3141061运行500预算；exec-weight v2等待3131858，未出方法结论 |
| M-03 | 容量、EMA、动作组和日程是否有额外收益 | 独立短对照，不与M-01/M-02同时改；EMA与原权重并列，可训练参数范围明确 | 待方法证据后选定具体配方 |
| M-04 | 离散动作监督及梯度隔离是否有效 | 区分纯FM、CE+FM无隔离、CE+FM有隔离；同数据来源，先梯度/无泄漏和可学习性门 | 原工程门通过，v1因前驱失败退出；同预算v2的FM3131867→joint3131878→KI3131887已核验等待/0更新，无自动追加 |
| AR-01 | 纯AR是否能学会且自由解码正确 | 原生/当前训练接口、codec/schema/完整动作组、自由生成而非仅teacher-forcing；短训后局部闭环 | 两种AR500均完成但自由0/5完整；marker v2等待3131895。schema十窗内容FM胜7/10，native十观察/一次神经raw23/msgpack门已通过。989a575/290 CPU及实际socket通过后，原生task-only/schema局部仿真3196694运行：原448prefix＋最多256模型控制/一个回合；尚无SR/结果，CoT正式训练未完成 |
