...
`e7478cf`收尾修正：APPROACH同样开放2mm的micro，避免1cm不可达时隐藏较小可行步；每个新的恢复尝试独立重置观察计时，包括已经处于RECOVER的情况。仍保留总恢复上限和全部控制约束。这两项只有CPU回归证据，尚未新跑物理闭环。
...
- 原Qwen3.5-4B作对照，官方Qwen3.8-27B固定revision `1d4bf0f2ff6012fd82039f2fa52739d0dd7c60c0`作强参考；没有训练或套用不匹配的Show-Harness adapter。下载55,586,036,737字节，文件大小校验完成；静态15状态中radio可见识别4B为2/10、27B为10/10，但两条27B物理短测均未成功。27B静态观察/动作中位8.14/3.43s、峰值52.36GiB；不是可用机器人策略或小模型Hz的保证。
- 静态与物理试验限额在[plan](plan.md) H-06中；静态集按已有状态人工审核，不把模型回答或模板补全当机器人成功。

完整结果及视频见[验收报告](experiments/2026-09-17-semantic-agent-v2.md)。最终e7478cf本地与robo全部64项CPU测试通过；新digest不能使用26de8ad的旧控制门放行。当前只是通过有界工程验证的开发实现：全局搜索、持物正例、正式部署和独立review仍待验证，不将这些待办描述为已解决。
