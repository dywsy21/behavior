...
A4于2026-09-13 07:39完成2500新增更新，checkpoint恢复验收通过，40000条实际train抽取；固定80诊断FM=0.1969439941，较A3约降0.92%，尚无新SR结论。接下来先配对原观察与动作，决定一个训练机制验证，见[实时计划](plan.md)。高层不因A4低层完成而自动视为训好；不启动全任务训练或队友的RL/数据扩充。中断前新建的`probe_a3_original_grasp_capacity_v2.py`及对应测试仍是未完成、未验证、未运行的草稿，不属于交付模型。
...
| A-02 | P0/A；本轮Codex | 关键动作不足：原状态、图像/本体、动作目标与条件响应分开查 | 单次有界学习验证；记录关键运动误差、抓取局部成功，而非只FM均值 | A4-2500完成；保存回读通过，固定80收益约0.92%；进行配对动作/条件响应与训练机制验证 |
