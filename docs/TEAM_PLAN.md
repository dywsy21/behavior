...
| H-07 | 提案/Codex | 通用主动观察/搜索覆盖、可达动作选择及抓取交接 | 固定新源码/预算重新过门；先目标—指尖可定位、搜索有覆盖、拒绝后选到可行步，再少量可信抓取/交接；不逐task手写轨迹 | 未启动，先团队review和新预算；不因“27B认得目标”自动SFT/扩大任务数，数据与RL队友不重复承担 |
