...
更新：2026-09-12。**本文件是当前任务板；旧长执行日志只作历史证据，不作为现行待办。**
...
`native_a3_aligned_development_pilot_v3`：同A3/B-final权重、同public_test301/环境seed0/policy17，只修推理动作约定。2026-09-12 20:50（北京时间）核对：task0完整3224动作、821.49秒、失败；task1正在跑，其余待执行。**不得把未完成的任务计入分母，不提前写成新0/5。** 该序列是上一个工作阶段已启动的对照，不另外复制启动第二轮。

当前改以仓库/磁盘/团队计划整理为优先；不新开训练。中断前新建的`probe_a3_original_grasp_capacity_v2.py`及对应测试是未完成、未验证、未运行的草稿，不属于交付模型。
...
| --- | --- | --- | --- | --- |
| P0-01 | P0/A | GitHub作为唯一代码/文档同步源，迁入远端主仓源码，归档旧本地文件 | main可pull/push；AGENTS跟踪；无数据/权重/秘密入库；原脏仓和归档可恢复 | 本轮实施中 |
| P0-02 | P0/A | 区分“主仓源码”和“最新实验快照”，整理后者差异 | 提供逐模块diff/测试，先merge修复再宣称主仓具备A3能力；活跃源码不热改 | 待办 |
| P0-03 | P0/轮值 | 迁移闲置大文件，争取源盘净释放2T | 真实df差值、迁移校验/清单/恢复命令；训练/数据/唯一结果不丢 | 本轮实施中 |
| A-01 | P0/A | mask/动作起点修复形成可复用回归 | 真实23D维度全保留；6图像≠5历史动作；同输入实际动作一致 | 实验runtime已通过，主仓待集成 |
| A-02 | P0/A | 关键动作不足：原状态、图像/本体、动作目标与条件响应分开查 | 最多一个小样本学习性/采样或损失改动对照；记录关键运动误差、抓取局部成功，而非只FM均值 | 待有界试验 |
...
在[协作流程](COLLABORATION.md)认领分支；在[服务器目录](SERVER_LAYOUT.md)查文件；历史细节到`docs/archive/`或该文档所指归档，不继续把几百行历史堆在任务板顶部。
