...
2026-09-18 21:26：H-09由Astra/max独立负责，GPU1；2B LoRA train_v1/140c47d实际600步/18.26min完成，最终adapter b5a125ed。192留出原始5/192、FT180/192，但速度规则170/192，操作仅2/18→7/18；非低base速度172例FT全对，低速20例仅8对，不能声称视觉掌握。975852a双工程门已过、初图人工核对；74735/8918服务≤160调用，原四匹配闭环开始，各40决策/1280新控制/1200秒，前缀单列。主代理已review数据/训练/live及诊断隔离，无未解决阻塞；另独立看4训练面板。≤8GiB新增/盘余≥80GiB，不再训练，不改队友数据/RL范围。详见H-09专属报告。
...
2026-09-18 11:54：H-07两真实控制门＋本人首帧检查完成，原9993fab唯一两策略`radio_27b_v1`/`plates_27b_v1`已启动（新H-07根，不是旧H-06同名run）。各48决策/1536新控制/1200s、共享固定27B GPU3服务；无训练，真实效果待验，不再新增reset。独立review/正式提交包装仍待；不改变队友分工。
...
| H-07 | P0/Codex | 通用主动观察/搜索覆盖、可达动作选择及抓取交接 | 固定新源码/预算重新过门；先目标—夹爪可定位、搜索有覆盖、拒绝后选到可行步，再少量可信抓取/交接；不逐task手写轨迹 | 9月18日用户批准，实施中；合法RGB-D、自身FK、有限搜索/重规划，不改变未完成目标。CPU→2控制门→6静态schema＋各1局部回合/总224调用；0训练，未有结果，独立review/正式包装仍待 |
