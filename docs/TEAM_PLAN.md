...
A4于2026-09-13 07:39完成2500新增更新，checkpoint恢复验收通过，40000条实际train抽取；固定80诊断FM=0.1969439941，较A3约降0.92%，尚无新SR结论。170次同输入对照已完成，两个seed的右臂均值误差约降7.99%/3.28%，明显运动窗优于原地保持仍都是10/16，收益不普遍。正在准备一次同起点/正确GRASP的1280模型控制L1复测，再决定训练机制验证，见[实时计划](plan.md)。高层不因A4低层完成而自动视为训好；不启动全任务训练或队友的RL/数据扩充。中断前新建的`probe_a3_original_grasp_capacity_v2.py`及对应测试仍是未完成、未验证、未运行的草稿，不属于交付模型。
