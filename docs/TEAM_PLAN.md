...
更新：2026-09-12。**本文件是三人协作任务板；goal执行总计划与实时工作记录统一维护在[plan.md](plan.md)。** 分工、预算或任务状态变化时两处同步；旧长执行日志只作历史证据，不作为现行待办。
...
`native_a3_aligned_development_pilot_v3`：同A3/B-final权重、同public_test301/环境seed0/policy17，只修推理动作约定。2026-09-12 21:48（北京时间）核对：task0完整3224动作、821.49秒、失败；task1完整7901动作、2035.98秒、失败；task2在约5280动作处继续，task3/4待执行。当前已完成部分为0/2。**不得把未完成的任务计入分母，不提前写成新0/5。** 该序列是上一个工作阶段已启动的对照，不另外复制启动第二轮。

2026-09-12晚用户明确授权启动一晚训练，随后取消墙钟限时：Codex认领A-02的单次A3低层继续学习验证，2500新增更新、无训练墙钟硬截止。GPU门、四卡5步保存回读通过；23:48进入正式训练，23:55四rank真实参数更新已核验，样本回执进入第30步，见[实时计划](plan.md)。高层不训练，不启动全任务训练、RL或未验收反馈头。中断前新建的`probe_a3_original_grasp_capacity_v2.py`及对应测试仍是未完成、未验证、未运行的草稿，不属于交付模型。

## 3. 三个人如何真正并行

以下是职责槽位，由三位成员各自认领，避免根据聊天身份猜测分工；每项只有一个负责人。A兼任串行集成人，B/C独立开发。

| 负责人 | 主线及文件所有权 | 本轮应交付什么 | 依赖 |
| --- | --- | --- | --- |
| A：低层与集成 | FM/动作processor/训练采样、加载与serving；维护可复现运行配置。修改`src/g05/models/g05/`低层文件、动作processor和相关tests | 将已验证mask/起点修复从实验快照整理进协作代码；定位正确意图下的接近/闭合/运输动作不足；一个有界的低层方法对照 | 先冻结23D动作、六帧历史及接口；不等反馈头才能诊断L1 |
| B：高层、记忆与反馈 | planner/技能协议、sidecar与outcome数据/模型；不改低层控制数学 | 可部署的完成反馈最小闭环；明确UNKNOWN；验证对象/手臂绑定、记忆更新、切换延迟；数据抽查与校准报告 | 用现有train回放先做数据/reader；不等A全任务成功。接口改动和A/C协商 |
| C：通用RL、仿真与评测 | 统一rollout/reward/credit-assignment、RL trainer及模拟器adapter；不热改A/B正在运行的源码 | πRL/ReinFlow式FM后训练可行性；共享奖励/重置协议；跨抓取、开合、运输/放置等异质技能的小预算验证 | 先固定修正版模型/接口；A协助FM概率/参数接入，B提供可信结果标签 |

并行方式：A查“正确技能→动作及FM可训练接入”，B查“过去观察→可信反馈/下一技能”，C查“多任务环境交互→统一奖励→通用RL更新”。三条线共享数据/事件协议，但不要求同时改同一个模型。先分别验证，再由A串行合并做少量协同闭环。
...
| A-02 | P0/A；本轮Codex | 关键动作不足：原状态、图像/本体、动作目标与条件响应分开查 | 单次有界学习验证；记录关键运动误差、抓取局部成功，而非只FM均值 | A4正式训练中；四rank有效参数更新已核验；2500新增更新、无墙钟硬截止；checkpoint/动作和留出效果待后续验收 |
| B-01 | P0/B | outcome最小可信训练集：抓稳、释放后放置、掉落事件 | train实例分组；过去观测输入；UNKNOWN不造标签；人工审查；独立实例校准 | 候选已有，未放行 |
| B-02 | P0/B | 完成反馈头训练与接线 | 无特权部署输入；误报成功率/覆盖率/切换延迟均有实测；不把阶段边界当成功 | 待办 |
| B-03 | P1/B | 高层对象/手臂/父目标绑定及记忆去重 | 同bundle刷新不重复写记忆；抓错/未知不伪写完成；低层成功后及时换阶段 | 有协议，真实效果待改进 |
