...
| 位置 | 内容/边界 |
| --- | --- |
| `/mnt/sdc1/robodojo/behavior_dev/semantic_agent_20260917` | H-02/03 run根；`gate_v1`是24命令控制门，`radio_2b_v2`/`radio_4b_v2`为局部VLM闭环；实际完成状态见各result/日志及plan，不凭目录存在认定成功 |
...
| 同根`server_*` / `bench_*` | 身份、调用清单/图像prompt hash和同图延迟；端口仅127.0.0.1的8897/8898，配额有界，结束释放自己的服务 |
| `/mnt/sdc1/robodojo/behavior_dev/git_worktrees/semantic_agent_*` | `1680e01`/`f18dd43`/`a7880ec`/`fa9db1b`/`d312380`各不可变Git快照；服务/仿真引用的快照不能热pull |
