...
class GroundedPolicy(VLMPolicy):
    ...
    def act_feasible(self,harness,state,bundle,allowed):
        ...
        if getattr(harness,"approach_translation_preview",False):
            system += " translation_after previews whether an already feasible1cm hand translation could unlock a currently blocked3cm reach. A setup move with smaller immediate gain may offer better two-command progress than repeatedly taking the largest immediate gain. Compare both gains and the raw images, not gain alone. This assumes a stationary surface and certifies only robot kinematics/self-collision, NOT future environment clearance, grasp pose or success. Choose ONLY one currently offered first translation; its predicted followup is NOT authorized or queued. Reobserve and recheck before the next command."
        if getattr(harness,"near_pose_gap",False):
            system += " When a near-field approach has no feasible improving arm translation, near_pose_gap_option marks small wrist pose changes or1cm retreats. A retreat deliberately increases surface distance to make room; a rotation may improve grasp orientation with almost no distance gain. Review the raw views and calibrated finger region, not point-distance gain alone. These options have an extra sampled visible-depth sweep veto, NOT complete environment or contact safety certification. Never infer that occlusion means safe or that target identity implies a pinchable grasp part. Select only one offered action, or HOLD if the image contradicts safety; a fresh depth check can still veto it."
        ...
        if harness.goal.kind=="press" and harness.grounding.get("press_reference"):
            system += " PRESS uses the fixed exposed finger point in press_reference, NOT the closing-gap centre or yellow grasp-region guide. Use target_minus_reference_base_m/tool_m and distance_to_active_reference_m for current geometry; the same link-local XYZ is used for candidate predictions. Do not reinterpret a smaller distance as physical contact, button activation or success. Keep the other loaded hand/grip protected; missing or blocked surface references permit no point-guided press. Tool-frame forward/back, left/right, up/down are +/- local X/Y/Z, not camera directions."
        elif getattr(harness,"contact_geometry",True):
