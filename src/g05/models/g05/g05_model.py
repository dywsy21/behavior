...
class G05Model(nn.Module):
    ...
    def forward(
        ...
        action_pad_masks: Optional[torch.BoolTensor],
        ...
        **kwargs,
        ...
        # MEM-Lite is AR-only and may not carry a continuous action tensor at all.
        # Do not enter the FM path in that mode: this keeps planner/action-token
        # batches cheap and prevents an accidental continuous-action gradient.
        if continuous_action:
            if actions is None or action_pad_masks is None:
                raise ValueError(
                    "actions and action_pad_masks are required when continuous_action=True"
                )
            ...
            fm_loss = torch.zeros((), device=device, dtype=vlm_hidden.dtype)
        if skip_ce_loss:
