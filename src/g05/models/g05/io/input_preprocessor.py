...
class InputPreprocessor:
    ...
    def __init__(
        ...
        # MEM-Lite high-level generation has an explicit boundary which is
        # distinct from EOV (the low-level action boundary).  Register it
    ...
    def _parse_template(self, template: str) -> List[TemplateSegment]:
        ...
        for match in pattern.finditer(template):
            ...
            # Step 2: preserve any registered single special token before
            # interpreting underscores as the sampleKey_processorKey grammar.
            # In particular, MEM-Lite uses <HL_END> as a learned literal target;
            # it must not be parsed as a fictitious sample key "HL"/processor
            # "END".
            special_tok = self.token_manager.resolve(raw_key)
            if special_tok is None:
                special_tok = _resolve_special_token(self.tokenizer, raw_key)
    ...
    def encode_train(
        ...
        **kwargs,
        ...
        # Truncate
        if any(sample.get("memlite_causal_prompt", False) for sample in samples) and input_ids.shape[1] > max_chunk_token_length:
            raise ValueError(
                "Causal MEM-Lite context exceeds max_chunk_token_length; refusing to truncate "
                f"memory, previous intent or current state ({input_ids.shape[1]} > {max_chunk_token_length})"
            )
    ...
    def decode_ar(
        ...
        for i, ids in enumerate(generated_ids):
            ...
            if action_only:
                # The separated MEM-Lite low-level branch passes only tokens
                # emitted after EOC. Do not parse a high-level prefix or
                # concatenate intent/memory IDs into this action string.
                cot_texts.append("")
                action_str = self._extract_action_str("Action: " + decoded_str)
            else:
