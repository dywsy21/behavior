...
def _select_action_samples_for_tokenizer_eval(samples):
    """Return low/action samples suitable for the first-batch VQ diagnostic.

    MEM-Lite deliberately mixes two different supervision formats in a physical
    dataloader batch. A high sample has only textual planner targets and must
    never be presented to the action tokenizer; a low sample has the usual
    action.value tensor.

    Returns (action_indices, high_indices, saw_memlite). saw_memlite lets the
    caller distinguish an intentional high-only batch from legacy samples, for
    which the historical gt_action fallback remains valid.
    """
    aliases = {
        "planner": "high",
        "high_level": "high",
    ...
    for index, sample in enumerate(samples):
        if not isinstance(sample, dict):
            raise TypeError(
                f"first-batch tokenizer diagnostic expects dict samples, got {type(sample)!r} "
                f"at index {index}"
            )
        raw_branch = sample.get("memlite_branch")
        branch = None
        if raw_branch is not None:
            saw_memlite = True
            branch = aliases.get(str(raw_branch).strip().lower(), str(raw_branch).strip().lower())
            if branch not in {"high", "low"}:
                raise ValueError(
                    f"invalid MEM-Lite branch {raw_branch!r} at sample {index}; expected high or low"
                )

        action_entry = sample.get("action")
        has_action_value = isinstance(action_entry, dict) and "value" in action_entry

        if branch == "high":
            # These are the three explicitly supervised planner targets in
            # MEMLiteHighLevelBuilder. Check their presence without ever
            # reading action: a high sample intentionally has none.
            missing = [
                field
                for field in ("intent", "memory_update", "intent_status")
                if not isinstance(sample.get(field), str) or not sample[field].strip()
            ]
            if missing:
                raise ValueError(
                    f"MEM-Lite high sample {index} is missing textual target(s): {missing}"
                )
            high_indices.append(index)
            continue

        if branch == "low" and not has_action_value:
            # Do not turn a malformed action sample into a quiet diagnostic
            # skip: that would hide an error in the MEM-Lite data/template
            # contract before the actual forward pass reaches it.
            raise KeyError(
                f"MEM-Lite low sample {index} is missing samples['action']['value']"
            )
        if has_action_value:
            action_indices.append(index)

    return action_indices, high_indices, saw_memlite
...
def eval_tokenizer_first_batch(action_tokenizer, batch, device_id, *, hf_tokenizer=None):
    ...
    # Try vlm_action from batch["samples"] (VLA path). For MEM-Lite,
    # action/tokenizer evaluation is meaningful only on low-level samples;
    # high-level planner samples contain text labels but no action entry.
    tokenizer_batch_indices = None
    samples = batch.get("samples")
    if samples is not None and isinstance(samples, list) and len(samples) > 0:
        action_indices, high_indices, saw_memlite = _select_action_samples_for_tokenizer_eval(
            samples
        )
        if high_indices:
            logger.info(
                "[First Batch Tokenizer Eval] MEM-Lite verified %d high-level "
                "text target(s); excluding them from action-tokenizer diagnostics.",
                len(high_indices),
            )
        if action_indices:
            first_sample = samples[action_indices[0]]
            action_entry = first_sample["action"]
            vlm_actions = [samples[index]["action"]["value"] for index in action_indices]
            tokenizer_input = torch.stack(vlm_actions, dim=0)
            input_source = "vlm_action"
            tokenizer_batch_indices = action_indices
            ...
            logger.info(
                "[First Batch Tokenizer Eval] MEM-Lite action diagnostic on %d low-level "
                "sample(s) (high=%d).",
                len(action_indices),
                len(high_indices),
            )
            ...
            logger.info(
                "[First Batch Tokenizer Eval] MEM-Lite batch has %d high-level sample(s) "
                "and no low-level action target; action-tokenizer diagnostic skipped.",
                len(high_indices),
            )
            return
    ...
    def _select_batch_rows(value):
        if (
            tokenizer_batch_indices is not None
            and isinstance(value, torch.Tensor)
            and value.ndim > 0
            and value.shape[0] == len(samples)
        ):
            return value.index_select(
                0,
                torch.tensor(tokenizer_batch_indices, dtype=torch.long, device=value.device),
            )
        return value

    action_op_mask = _select_batch_rows(action_op_mask)
    action_dim_is_pad = _select_batch_rows(action_dim_is_pad)
    ...
    frequency = _select_batch_rows(batch.get("frequency"))
