...
class ActionNormalizationError(ValueError):
    """A predicted action cannot be safely inverse-normalized."""
...
NormMode = Union[
    Literal[
        ...
        "z-score-tail-bounded",  # same training representation; bounded inverse
...
class SingleFieldLinearNormalizer(BaseActionStateTransform):
    ...
    def __init__(
        ...
        self.stats = stats
        self.mode = mode
        self.bounded_backward = mode.endswith("-bounded")
        self.last_backward_diagnostics = {}
        ...
        transform_mode = mode.removesuffix("-bounded")
        _is_tail = transform_mode.endswith("-tail")
        _base_mode = transform_mode[:-5] if _is_tail else transform_mode
        if self.bounded_backward:
            lo, hi = stats.get("min"), stats.get("max")
            if lo is None or hi is None:
                raise ValueError("Bounded inverse requires train-only min/max statistics")
            if not (torch.isfinite(lo).all() and torch.isfinite(hi).all() and (lo <= hi).all()):
                raise ValueError("Bounded inverse requires finite, ordered min/max statistics")
    ...
    def _apply_tail_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        q01 = self._field_for_input(self._tail_q01, x)
        q99 = self._field_for_input(self._tail_q99, x)
        c_pos = self._field_for_input(self._tail_c_pos, x)
        c_neg = self._field_for_input(self._tail_c_neg, x)
    ...
    def _apply_tail_backward(self, y: torch.Tensor) -> torch.Tensor:
        ...
        q01 = self._field_for_input(self._tail_q01, y)
        q99 = self._field_for_input(self._tail_q99, y)
        c_pos = self._field_for_input(self._tail_c_pos, y)
        c_neg = self._field_for_input(self._tail_c_neg, y)
    ...
    @staticmethod
    def _field_for_input(field: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        """Match stepwise bounds as well as scale/offset to a shorter horizon."""
        field = field.to(x.device)
        if field.ndim >= 2 and x.ndim >= 2:
            if x.shape[-2] > field.shape[-2]:
                raise ValueError("Input horizon exceeds the available normalization statistics")
            field = field[..., :x.shape[-2], :]
        return field
    ...
    def backward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        self.last_backward_diagnostics = {}
        if self.bounded_backward:
            if not torch.isfinite(x_main).all():
                raise ActionNormalizationError("Non-finite decoded action before inverse normalization")
            lo = self._field_for_input(self.stats["min"], x_main)
            hi = self._field_for_input(self.stats["max"], x_main)
            # Bound in compressed space BEFORE expm1, not after an overflow.
            # Merely clipping a z-score to [-5,5] is insufficient: even a
            # finite z-score in that range can become a huge physical delta.
            lo_compressed = self._apply_tail_forward(lo) if self._tail_c_pos is not None else lo
            hi_compressed = self._apply_tail_forward(hi) if self._tail_c_pos is not None else hi
            norm_lo = lo_compressed * scale + offset
            norm_hi = hi_compressed * scale + offset
            bounded = torch.maximum(torch.minimum(x_main, norm_hi), norm_lo)
            distance = (x_main - bounded).abs()
            self.last_backward_diagnostics = {
                "count": int(x_main.numel()),
                "clipped_count": int((distance > 0).sum().item()),
                "max_normalized_excess": float(distance.max().item()) if distance.numel() else 0.0,
                "decoded_abs_max": float(x_main.abs().max().item()) if x_main.numel() else 0.0,
            }
            x_main = bounded
        if self.mode == "tanh":
        ...
        if self.bounded_backward:
            # Cover floating-point roundoff at expm1 endpoints, too.
            x_main = torch.maximum(torch.minimum(x_main, hi), lo)
            if not torch.isfinite(x_main).all():
                raise ActionNormalizationError("Non-finite action after bounded inverse normalization")
            self.last_backward_diagnostics["delta_abs_max"] = float(x_main.abs().max().item()) if x_main.numel() else 0.0
