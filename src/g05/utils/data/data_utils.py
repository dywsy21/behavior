...
def custom_collate_fn(batch):
    ...
    # MEM-Lite mixed batches intentionally contain branch-specific keys
    # (high-level memory targets vs low-level action targets).  Iterate over
    # the union and preserve ``None`` for a field absent from one branch;
    # assuming every sample has the first sample's keys raises KeyError during
    # the first mixed batch.
    keys = list(dict.fromkeys(key for item in batch for key in item.keys()))
    for key in keys:
        ...
        elif all(isinstance(value, dict) for value in values if value is not None) and any(
            isinstance(value, dict) for value in values
        ):
            collated[key] = custom_collate_fn([value or {} for value in values])
