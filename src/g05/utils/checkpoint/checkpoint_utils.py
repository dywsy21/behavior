...
def save_training_checkpoint(
    ...
    **extra_state,
    ...
    checkpoint. Resumable callers pass both optimizer and scheduler; extra
    keyword args (e.g. ``action_batch_idx``) are stored verbatim in the
    checkpoint dict.
    """
...
def checkpoint_save_reason(
    step: int,
    max_steps: int,
    checkpointing_steps: Optional[int],
) -> Optional[str]:
    """Return the one reason to persist a resumable checkpoint after an optimizer step.

    A periodic step that is also the final step intentionally returns only
    "periodic": the caller saves once, and that one checkpoint is both the
    periodic and final resume point. Disabled intervals (None or 0) still save
    the final completed step. Negative intervals are invalid.
    """
    interval = int(checkpointing_steps) if checkpointing_steps is not None else 0
    if interval < 0:
        raise ValueError(f"checkpointing_steps must be non-negative, got {interval}")
    if step <= 0:
        return None
