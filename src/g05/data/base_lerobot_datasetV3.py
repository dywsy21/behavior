...
def _build_global_stride_anchors(
    episode_ranges: Tuple[Tuple[int, int], ...], downsample_rate: int
) -> torch.Tensor:
    """Build globally stride-aligned anchors without materializing a frame map.

    Each physical episode interval is sampled independently.  This keeps every
    anchor inside the train selection even when a task-stratified split has
    non-contiguous episode ranges.  Very short intervals still contribute their
    first frame so every selected episode is represented.
    """
    ...
    for start, end in episode_ranges:
        ...
        first = start + ((-start) % downsample_rate)
        if first >= end:
            chunks.append(torch.tensor([start], dtype=torch.long))
        else:
            chunks.append(torch.arange(first, end, downsample_rate, dtype=torch.long))
    if not chunks:
        return torch.empty(0, dtype=torch.long)
    return torch.cat(chunks)


def _frame_indices_in_ranges(
    frame_indices: torch.Tensor, episode_ranges: Tuple[Tuple[int, int], ...]
) -> torch.Tensor:
    """Return a boolean membership mask for sorted, non-overlapping intervals."""
    if frame_indices.ndim != 1:
        raise ValueError(f"frame_indices must be 1D, got shape {tuple(frame_indices.shape)}")
    if not episode_ranges:
        return torch.zeros_like(frame_indices, dtype=torch.bool)

    starts = torch.tensor([start for start, _ in episode_ranges], dtype=torch.long)
    ends = torch.tensor([end for _, end in episode_ranges], dtype=torch.long)
    positions = torch.searchsorted(ends, frame_indices, side="right")
    valid = positions < len(episode_ranges)
    if valid.any():
        valid_indices = torch.nonzero(valid, as_tuple=False).squeeze(1)
        valid_positions = positions[valid_indices]
        valid[valid_indices] = (
            (frame_indices[valid_indices] >= starts[valid_positions])
            & (frame_indices[valid_indices] < ends[valid_positions])
        )
    return valid
...
class BaseLerobotDatasetV3(BaseLerobotDataset):
    ...
    def _fast_get_dataset_stats(
        ...
        selected_frame_count = sum(end - start for start, end in stats_episode_ranges)
        _stats_write(
            "🔒 [Stats] Selection: {} episodes, {} selected physical frames before downsampling".format(
                _format_count(len(stats_episode_ids)),
                _format_count(selected_frame_count),
            ),
            "yellow",
        )
        ...
        if ep_starts is not None and N_from_meta > 0:
            ...
            N = N_from_meta
            sampled_base_idx = _build_global_stride_anchors(
                stats_episode_ranges, self.stats_downsample_rate
            )
            ...
            if not _frame_indices_in_ranges(required_indices, stats_episode_ranges).all():
                raise RuntimeError(
                    "Stats sampling plan escaped the selected episode ranges; refusing to "
                    "compute normalization statistics with held-out rows"
                )
            _stats_write(
                "🧮 [Stats] Sample plan: {} base anchors -> {} required raw frames ({} of selected frames)".format(
                    _format_count(sampled_base_idx.numel()),
                    _format_count(required_indices.numel()),
                    _format_ratio(required_indices.numel(), selected_frame_count),
                ),
                "yellow",
            )
