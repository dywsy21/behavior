...
class MixtureLerobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        with tqdm(
            ...
            for emb in embodiment_datasets:
                ...
                ds_val_split_mode = emb_ds_cfg.pop("val_split_mode", val_split_mode)
                ...
                for group_idx, group in enumerate(dataset_groups):
                    ...
                    dataset = get_obj_from_str(dataset_type)(
                        **emb_ds_cfg,
                        ...
                        val_split_mode=ds_val_split_mode,
    ...
    def get_memlite_branch_sampling_spec(self) -> Dict[str, Any]:
        """Expose compact high keys and low ranges in the mixture view.

        Weighted logical views would require expanding every low range, so the
        branch-balanced sampler rejects them rather than silently sampling a
        different distribution.  MEM-Lite task configs use unweighted groups.
        """
        cached = getattr(self, "_memlite_branch_sampling_spec", None)
        if cached is not None:
            return cached
        if self.is_training_set and self.use_weight_for_sampling:
            raise ValueError(
                "MEM-Lite branch-balanced sampling requires use_weight_for_sampling=false "
                "so low ranges remain compact and exact"
            )
        result: Dict[str, Any] = {"high": [], "low_ranges": []}
        for dataset_idx, dataset in enumerate(self.datasets):
            if not hasattr(dataset, "get_memlite_branch_sampling_spec"):
                raise TypeError(
                    f"Dataset {dataset_idx} does not support MEM-Lite branch sampling"
                )
            child = dataset.get_memlite_branch_sampling_spec()
            offset = int(self.effective_starts[dataset_idx])
            result["high"].extend(offset + int(index) for index in child["high"])
            result["low_ranges"].extend(
                (offset + int(start), offset + int(end))
                for start, end in child["low_ranges"]
            )
        if not result["high"] or not result["low_ranges"]:
            raise RuntimeError(
                "MEM-Lite mixture needs at least one valid high and low sample; "
                f"got high={len(result['high'])}, low_ranges={len(result['low_ranges'])}"
            )
