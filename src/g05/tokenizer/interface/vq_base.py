...
class VQActionTokenizer(BaseActionTokenizer):
    ...
    def __init__(
        ...
        *,
        ...
        self.require_group_markers = bool(vq_config.get("require_group_markers", False))
    ...
    def _decode_action_indices(
        ...
        *,
        ...
        if isinstance(self.serializer, GroupedFlatSerializer):
            ...
            if getattr(self, "require_group_markers", False) and any(
                not any(t in marker_values for t in row) for row in action_indices
            ):
                from g05.utils.memlite_protocol import ActionDecodeError
                raise ActionDecodeError("Missing AR action group markers; refusing synthetic zero-action decode")
            if (
                has_markers
