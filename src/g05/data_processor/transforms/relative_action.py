...
class BehaviorPerKeyTransform(BaseActionStateTransform):
    ...
    Operations:
      ...
      3. Merge lower body action: trunk_qpos(4) + base_qvel(3) → lower_body(7)
    ...
    supports_raw_state_anchor = True
    ...
    def forward(self, batch: Dict):
        ...
        if "trunk_qpos" in action and "base_qvel" in action:
            ...
            # action_op_mask is created by the filter before this transform and
            # therefore still uses the raw keys. Keep its key structure in lock
            # step with the numeric action; otherwise GroupedPaddingMerger treats
            # lower_body as a virtual (all-false) part and AR drops its tokens.
            if (
                isinstance(action_op_mask, dict)
                and "trunk_qpos" in action_op_mask
                and "base_qvel" in action_op_mask
            ):
                action_op_mask["lower_body"] = torch.cat(
                    [action_op_mask["trunk_qpos"], action_op_mask["base_qvel"]], dim=-1
                )
                del action_op_mask["trunk_qpos"]
                del action_op_mask["base_qvel"]
    ...
    @staticmethod
    def _relative_anchor(batch, key, state, action):
        legacy = state[..., -1:, :] if state.ndim == action.ndim else state
        anchors = batch.get("_raw_state_anchor")
        if anchors is None:
            return legacy
        if not isinstance(anchors, dict) or key not in anchors:
            raise ValueError(f"Missing raw relative-action anchor for {key}")
        anchor = torch.as_tensor(anchors[key], device=action.device, dtype=action.dtype)
        expected_shape = (*action.shape[:-2], 1, action.shape[-1])
        if tuple(anchor.shape) != expected_shape or not torch.isfinite(anchor).all():
            raise ValueError(f"Invalid raw {key} anchor: expected finite {expected_shape}, got {tuple(anchor.shape)}")
        batch.setdefault("_state_anchor_diagnostics", {})[key] = {
            "source": "raw_observation",
            "raw_abs_max": float(anchor.abs().max()),
            "normalized_roundtrip_error_max": float((anchor - legacy.to(anchor)).abs().max()),
        }
        return anchor
    ...
    def backward(self, batch: Dict):
        ...
        if "lower_body" in action and "trunk_qpos" in state:
            trunk_state_last = self._relative_anchor(
                batch, "trunk_qpos", state["trunk_qpos"], action["lower_body"][..., :4])
            for d in self.trunk_relative_dims:
                action["lower_body"][..., d] = (
                    action["lower_body"][..., d] + trunk_state_last[..., d]
                )

        # Reverse 3: split the transformed lower body back into raw action keys.
        # Do this after restoring the trunk-relative dimensions above so the raw
        # trunk target exactly matches the dataset representation.
        if "lower_body" in action:
            lb = action["lower_body"]
            action["trunk_qpos"] = lb[..., :4]
            action["base_qvel"] = lb[..., 4:]
            del action["lower_body"]

        # Restore the raw operational-mask keys using the same [trunk, base]
        # partition as forward. The mask can be absent in inference batches.
        action_op_mask = batch.get("action_op_mask")
        if isinstance(action_op_mask, dict) and "lower_body" in action_op_mask:
            lower_body_mask = action_op_mask["lower_body"]
            action_op_mask["trunk_qpos"] = lower_body_mask[..., :4]
            action_op_mask["base_qvel"] = lower_body_mask[..., 4:]
            del action_op_mask["lower_body"]

        # Reverse 2: undo relative
        for k in ("left_arm", "right_arm"):
            if k in action and k in state:
                state_last = self._relative_anchor(batch, k, state[k], action[k])
                action[k] = action[k] + state_last
