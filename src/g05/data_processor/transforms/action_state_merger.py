...
class GroupedPaddingMerger(BaseActionStateTransform):
    ...
    def _backward_one(self, flat_tensor, layout, residual_keys, embodiment_meta, max_shape_meta):
        """Split a flat action/state tensor or 1D action mask back to raw keys."""
        assert flat_tensor.ndim in (1, 2, 3)
        result = {}
        idx = 0
        ...
        for group in layout:
            ...
            for m in embodiment_meta:
                ...
                raw_keys = {m["key"] for m in embodiment_meta}
                if group.name == "lower_body" and {"trunk_qpos", "base_qvel"} <= raw_keys:
                    # BehaviorPerKeyTransform creates this intermediate semantic
                    # key from the two raw chassis fields. Preserve exactly this
                    # slot so its inverse can split it back. Other unmatched
                    # groups are virtual parts for a different embodiment and
                    # must remain absent rather than becoming fake predictions.
                    result[group.name] = slot_t
