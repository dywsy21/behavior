...
class GalaxeaCoTProcessor(FullProcessor):
    ...
    def _process_tensors(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 3. action & state
        data = self.preprocess_action_state(data, capture_gt_action=True)
        ...
        gt_action = data.pop("gt_action", None)
        if gt_action is not None:
            sample["gt_action"] = gt_action

        sample = self.build_action_sample(data, sample)
        sample["idx"] = data["idx"]
        # Keep routing/provenance fields next to the collated sample.  The
        # nested ``samples`` dict also carries memlite_branch, but retaining
        # these fields here lets samplers and diagnostics filter invalid/gap
        ...
        for meta_key in (
            ...
            "intent_status",
            "memlite_ignore",
            "memlite_label_missing",
            "memlite_segment_end",
            "memlite_action_horizon_end",
            "memlite_action_valid_steps",
            "step_is_qualified",
            "execution_feedback",
        ):
            if meta_key in data:
    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Both training and serving consume the nested sample. Keeping this
        # only on the outer collated record silently drops recovery feedback.
        for key in ("execution_feedback", "memlite_velocity_override"):
            if key in data:
                sample["samples"][key] = data[key]
