...
class BaseProcessor(ABC):
    def __init__(
        ...
        # ``None`` retains the historical open-loop behavior: the first
        # execution action follows the observation window.  Serving can set an
        # explicit offset when a checkpoint's action labels have no history.
        self.action_execution_start_index: int | None = None
    ...
    def set_action_execution_start_index(self, index: int | None):
        """Choose which decoded action timestep is sent to the environment.

        ``None`` intentionally preserves the legacy ``num_obs_steps - 1``
        convention.  Checkpoints with future-only action labels must instead
        supply their true action-history length explicitly (zero for the
        BEHAVIOR MEM-Lite run), rather than inferring it from image history.
        """
        if index is None:
            self.action_execution_start_index = None
            return self
        if isinstance(index, bool) or not isinstance(index, Integral):
            raise TypeError(
                "action_execution_start_index must be an integer or None, "
                f"got {index!r}"
            )
        if int(index) < 0:
            raise ValueError(
                "action_execution_start_index must be non-negative, "
                f"got {index}"
            )
        self.action_execution_start_index = int(index)
        return self

    def _slice_execution_actions(self, actions: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """Slice every raw action part at one validated execution offset."""
        if not isinstance(actions, dict) or not actions:
            raise ValueError("postprocess requires a non-empty per-part action dictionary")
        horizons: dict[str, int] = {}
        for key, value in actions.items():
            if not isinstance(value, torch.Tensor) or value.ndim < 3:
                raise ValueError(
                    "postprocess action part must be a [batch, horizon, dim] tensor: "
                    f"{key}={type(value).__name__} shape={getattr(value, 'shape', None)}"
                )
            horizons[str(key)] = int(value.shape[1])
        unique_horizons = set(horizons.values())
        if len(unique_horizons) != 1:
            raise ValueError(
                "postprocess action parts disagree on decoded horizon: "
                f"{horizons}"
            )
        horizon = unique_horizons.pop()
        start = (
            self.num_obs_steps - 1
            if self.action_execution_start_index is None
            else self.action_execution_start_index
        )
        if isinstance(start, bool) or not isinstance(start, Integral):
            raise TypeError(
                "resolved action execution start must be an integer, "
                f"got {start!r}"
            )
        start = int(start)
        if not 0 <= start < horizon:
            raise ValueError(
                "action_execution_start_index is outside decoded action horizon: "
                f"start={start}, horizon={horizon}, per_part={horizons}"
            )
        return dict_apply(actions, lambda value: value[:, start:, :])
    ...
    def postprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data["action"] = self._slice_execution_actions(data["action"])
        return data
...
class ActionProcessor(BaseProcessor):
    ...
    def preprocess_action_state(
        self, data: Dict[str, Any], *, capture_gt_action: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        # Open-loop targets must use the same semantic layout as the action
        # tokenizer, but remain in pre-normalization space. This is especially
        # important for BEHAVIOR where the transform creates lower_body from
        # trunk_qpos + base_qvel.
        gt_action = None
        if capture_gt_action and "action" in data:
            gt_action = self.action_state_merger.forward(deepcopy(data))["action"]
    ...
    def postprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if has_fm:
            ...
            if data.get("_raw_state_anchor") is not None:
                fm_data["_raw_state_anchor"] = data["_raw_state_anchor"]
            ...
            data["action_fm"] = self._slice_execution_actions(fm_data["action"])
            if fm_data.get("_state_anchor_diagnostics"):
                data["_state_anchor_diagnostics"] = fm_data["_state_anchor_diagnostics"]
        ...
        if has_ar:
            ...
            if data.get("_raw_state_anchor") is not None:
                ar_data["_raw_state_anchor"] = data["_raw_state_anchor"]
            ...
            data["action_ar"] = self._slice_execution_actions(ar_data["action"])
            if ar_data.get("_state_anchor_diagnostics"):
                data["_state_anchor_diagnostics"] = ar_data["_state_anchor_diagnostics"]
...
class FullProcessor(ActionProcessor):
    ...
    def __init__(
        ...
        if image_history_mode not in {"legacy", "preserve_cameras"}:
            raise ValueError(f"Unknown image_history_mode: {image_history_mode!r}")
        self.image_history_mode = image_history_mode
    ...
    def build_pixel_values(self, pixel_values: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        """
        if getattr(self, "image_history_mode", "legacy") == "preserve_cameras":
            # num_output_cameras is the number of camera feature streams after
            # the temporal ViT, NOT a budget of raw frames. The MEM encoder
            # fuses each camera's history and keeps its latest-frame features.
            # Flattening/selecting here both changes camera identity and turns
            # num_frames into one, silently disabling that encoder path.
            if len(pixel_values) != self.num_output_cameras:
                raise ValueError(
                    "preserve_cameras requires one output stream per camera: "
                    f"{len(pixel_values)} != {self.num_output_cameras}"
                )
            for key, frames in pixel_values.items():
                if frames.ndim != 4 or frames.shape[0] != self.num_obs_steps:
                    raise ValueError(
                        f"Camera {key!r} must retain {self.num_obs_steps} chronological "
                        f"frames [T,C,H,W], got {tuple(frames.shape)}"
                    )
            return pixel_values
