...
class MEMLiteHighLevelBuilder(MemorySamplesBuilder):
    """High-level MEM-Lite target: memory input -> intent/memory/status.

    This builder deliberately has no ``<action_action>`` placeholder. It is
    safe for planner-only CE batches and cannot accidentally train the AR
    action decoder on a textual action representation. Previous memory is a
    masked input; the three fields after EOC are supervised targets.

    Expected fields are ``memory`` (previous summary), ``intent`` (next
    subtask), ``memory_update`` (new summary), and ``intent_status``. Existing
    parquet tables may use ``atomic_task`` and ``status``; both are accepted as
    ...
    def template(self) -> str:
        return (
            ...
            "Memory: <memory_text_!> State: <proprio_proprio_!>;"
            "<chat_user_suffix><chat_assistant_prefix>"
            "<prompt_text_!><EOC><intent_text>|<memory_update_text>|"
            "<intent_status_text>|<HL_END><EOV>"
        )

    def can_handle(self, data: Dict[str, Any]) -> bool:
        if not super().can_handle(data):
            return False
        intent = data.get("intent", data.get("atomic_task"))
        status = data.get("intent_status", data.get("status", "CONTINUE"))
        return self._check_fields({"intent": intent, "status": status}, ("intent", "status"))

    def _populate_extra_samples(self, data, samples):
        super()._populate_extra_samples(data, samples)
        samples["memlite_branch"] = "high"
        samples["previous_intent"] = data.get("previous_intent") or "None"
        intent = data.get("intent", data.get("atomic_task", ""))
        memory_update = data.get("memory_update", "")
        status = data.get("intent_status", data.get("status", "CONTINUE"))
        samples["intent"] = f"Intent: {intent}"
        samples["memory_update"] = f"Updated Memory: {memory_update}"
        samples["intent_status"] = f"Status: {status}"
        samples["prompt"] = "Output the next intent, updated memory, and status:"


class MEMLiteCausalHighLevelBuilder(MEMLiteHighLevelBuilder):
    """v5 planner context; legacy v4 prompts remain unchanged."""

    @property
    def template(self) -> str:
        return super().template.replace(
            " State: <proprio_proprio_!>",
            " Previous intent: <previous_intent_text_!> State: <proprio_proprio_!>",
        )

    def _populate_extra_samples(self, data, samples):
        super()._populate_extra_samples(data, samples)
        samples["memlite_causal_prompt"] = True


class IntentActionBuilder(BaseSamplesBuilder):
    """Low-level MEM-Lite target: masked intent input -> AR action tokens.

    ``intent`` is conditioning context (``_!``), so no intent text
    contributes to CE. The only unmasked target after EOC is the serialized
    action token sequence. ``atomic_task`` is accepted as a legacy alias.
    """

    required_fields = ("intent",)
    eval_required_fields = ("intent",)
    ...
    def template(self) -> str:
        return (
            "<chat_user_prefix>" + self._images + "<bos>"
            "Embodiment: <embodiment_text_!>; Task: <command_text_!_200> "
            "Intent: <intent_text_!> State: <proprio_proprio_!>;"
            "<chat_user_suffix><chat_assistant_prefix>"
            "Action: <EOV><EOC><action_action>|<eos>"
        )

    def can_handle(self, data: Dict[str, Any]) -> bool:
        value = data.get("intent", data.get("atomic_task"))
        from g05.utils.memlite_protocol import is_terminal_intent
        if value and is_terminal_intent(str(value)):
            return False
        return self._check_fields({"intent": value}, ("intent",))

    def can_handle_for_eval(self, data: Dict[str, Any]) -> bool:
        return self.can_handle(data)

    def _populate_extra_samples(self, data, samples):
        samples["intent"] = data.get("intent", data.get("atomic_task", ""))
        samples["memlite_branch"] = "low"


class FMIntentActionBuilder(BaseSamplesBuilder):
    """Keep the old task-only FM prefix; carry intent ONLY as adapter metadata.

    There is deliberately no action placeholder, so neither training nor
    inference constructs action tokens. Continuous targets remain in the batch.
    """

    required_fields = ("intent",)
    eval_required_fields = ("intent",)

    def __init__(self, *args, high_builder=None, **kwargs):
        # Hydra merges the old mixed-builder dictionary; an explicit null
        # removes that branch without accepting a silently unused planner.
        if high_builder is not None:
            raise ValueError("FM low builder cannot contain a high-level builder")
        super().__init__(*args, **kwargs)

    @property
    def template(self) -> str:
        return super().template.replace("<action_action>|", "")

    def can_handle(self, data):
        from g05.utils.memlite_protocol import is_terminal_intent
        intent = str(data.get("intent", "") or "").strip()
        return data.get("memlite_branch") == "low" and bool(intent) and not is_terminal_intent(intent)

    def can_handle_for_eval(self, data):
        return self.can_handle(data)

    def _populate_extra_samples(self, data, samples):
        if not self.can_handle(data):
            raise ValueError("FM adapter needs a nonterminal, causally labelled low-level row")
        samples["intent"] = str(data["intent"])
        samples["memlite_branch"] = "low"


class MEMLiteMixedBuilder(BaseSamplesBuilder):
    """Route each annotated sample to the MEM-Lite high or low template.

    data[memlite_branch] must be exactly high/low (planner/action aliases
    accepted). This builder never falls back to the base template: missing
    annotations fail fast so a MEM-Lite run cannot silently train one branch.
    """

    required_fields = ("memlite_branch",)
    eval_required_fields = ("memlite_branch",)

    def __init__(self, num_input_images, image_sizes, embodiment_type=None,
                 high_builder=None, low_builder=None, **kwargs):
        super().__init__(num_input_images, image_sizes, embodiment_type, **kwargs)
        self._high = self._resolve_builder(high_builder, MEMLiteHighLevelBuilder,
                                           num_input_images, image_sizes, embodiment_type, kwargs)
        self._low = self._resolve_builder(low_builder, IntentActionBuilder,
                                          num_input_images, image_sizes, embodiment_type, kwargs)

    @staticmethod
    def _resolve_builder(spec, default_cls, num_input_images, image_sizes, embodiment_type, kwargs):
        from pydoc import locate
        child_cfg = {k: v for k, v in kwargs.items()
                     if k in ("hardcode_proprio_pad_zeros", "hardcode_action_pad_ones", "hardcode_instruction")}
        if spec is None:
            cls, cfg = default_cls, child_cfg
        else:
            try:
            ...
            if isinstance(spec, dict):
                cls = locate(spec.get("_target_"))
                if cls is None:
                    raise ImportError("Cannot locate MEM-Lite builder: %s" % spec.get("_target_"))
                cfg = {k: v for k, v in spec.items() if not k.startswith("_")}
                cfg.update(child_cfg)
            else:
                cls, cfg = spec, child_cfg
        return cls(num_input_images=num_input_images, image_sizes=image_sizes,
                   embodiment_type=embodiment_type, **cfg)

    @property
    def template(self):
        raise RuntimeError("MEMLiteMixedBuilder dispatches to a branch-specific template")

    @BaseSamplesBuilder.embodiment_type.setter
    def embodiment_type(self, value):
        self._embodiment_type = value
        self._high.embodiment_type = value
        self._low.embodiment_type = value

    def set_training(self, training):
        self._training = training
        for builder in (self._high, self._low):
            if hasattr(builder, "set_training"):
                builder.set_training(training)

    @staticmethod
    def _branch(data):
        value = data.get("memlite_branch")
        if value is None:
            raise ValueError("MEM-Lite sample requires data[memlite_branch] (high or low)")
        value = str(value).strip().lower()
        aliases = {"planner": "high", "high_level": "high", "action": "low", "low_level": "low"}
        value = aliases.get(value, value)
        if value not in {"high", "low"}:
            raise ValueError(f"invalid MEM-Lite branch {value!r}; expected high or low")
        return value

    def can_handle(self, data):
        branch = self._branch(data)
        return self._high.can_handle(data) if branch == "high" else self._low.can_handle(data)

    def can_handle_for_eval(self, data):
        branch = self._branch(data)
        return self._high.can_handle_for_eval(data) if branch == "high" else self._low.can_handle_for_eval(data)

    def build(self, data, sample):
        branch = self._branch(data)
        builder = self._high if branch == "high" else self._low
        if not builder.can_handle(data):
            required = getattr(builder, "required_fields", ())
            raise ValueError(
                f"MEM-Lite {branch} sample cannot be built: required fields "
                f"{tuple(required)!r} are missing or invalid"
            )
        out = builder.build(data, sample)
        out["memlite_branch"] = branch
        return out
